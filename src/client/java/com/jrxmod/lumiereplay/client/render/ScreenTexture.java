...
public class ScreenTexture implements AutoCloseable {
    ...
    private volatile int                   uploadThrottle = 1;
    ...
    public void upload() {
        if (uploadThrottle <= 1) { scheduleUpload(); return; }
        if (++uploadTick % uploadThrottle == 0) scheduleUpload();
    }

    public void setUploadThrottle(int every) {
        this.uploadThrottle = Math.max(1, every);
    }
