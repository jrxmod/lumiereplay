    ...
    public static void initialize() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(ProjectorUpdatePayload.ID, (payload, context) -> {
            context.server().execute(() -> {
                ...
                // Validate access rights — silently drop packet if not allowed
                if (!projector.canInteract(context.player())) {
                    LumierePlay.LOGGER.warn("Player {} denied access to projector at {}",
                        context.player().getName().getString(), pos);
                    return;
                }
                ...
                // Owner can change access mode
                boolean isOwner = projector.getOwnerUuid().isEmpty()
                    || projector.getOwnerUuid().equals(context.player().getUuidAsString());
                if (isOwner) projector.setAccessMode(payload.accessMode());
